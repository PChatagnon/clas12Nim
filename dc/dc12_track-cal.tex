...
tables, as well as specialized alignment procedures we undertook to most accurately determine
the wire positions themselves.
...
shift of the recorded hit time to larger values that depends on the
ion density of the track (velocity or $\beta$-dependent) and the distance of the track to the wire. 
...
A best fit to a scatterplot of TRKDOCA vs. time should define the time to
...
\begin{figure*}[htpb]
\vspace{6.5cm} 
\special{psfile=img/garfield.eps hscale=70 vscale=70 hoffset=20 voffset=-200}
...
\item the drift velocity is a function of electric field strength, so the point of minimum velocity is located at the point of
minimum field (and thus the inflection point on the $t$ vs. $x$ curve). 
...
scales like the square of the magnetic field strength $B^2$) can simply be added to the nominal functional form.
\end{itemize}

This single functional form is used to fill two tables: 
\begin{enumerate}
\item a table of time indexed by distance for use in the simulation;
...
mixture of 90\% argon - 10\% CO$_2$.  We call this parameter $V_0$.
...
($\geq$1000~V/cm)) and $V_{mid}$ (the drift velocity at the point of minimum electric field strength
($\approx$~250~V/cm))
...
of distance vs. time.  When the time is equal to $Tmax$ the distance is equal to
the largest value for the local angle, namely, $Dmax_{\alpha}$.  Also note that by
...
a signal hit with the same time $Tmax$ even though their DOCA
...
We derived the function for time vs. distance for a particular local angle $\alpha$ by
assuming the same functional form as for $\alpha = 30^\circ$ but 
with a different coefficient $d$, which 
...
\begin{enumerate}
\item for each angle $\alpha$ the time is $Tmax$ at $Dmax_{\alpha}$;
...
where $\alpha_0$ is the actual entrance angle, $b_{rot}$ is a parameter equal to 0.02$^\circ$/T, and $B$ is the
magnetic field strength in Tesla.

%%%%%%%%% Figure : TRKDOCA vs. Drift Time -- Angle and Field Dependence %%%%%%%%%%
\begin{figure*}[htb]
\vspace{10.0cm} 
\special{psfile=img/tvsx.eps hscale=60 vscale=60 hoffset=65 voffset=0}
\caption{\small{Scatterplot of the corrected drift time vs. TRKDOCA for 
(upper left) R1, showing curves for various local angles from 30$^{\circ}$
(right-most curve) to 0$^{\circ}$ (left-most curve).  (Upper right) for R2; 
...
value of approximately 100~ns/T$^2$ and $B$ is the magnetic field strength in Tesla. In this expression, the
...
outside the torus cryostats in regions where field strengths are less than 0.2~T.  
...
a histogram of TRKDOCA vs. time, storing the fit parameters in a database, re-doing
the track fitting, and iterating.

To illustrate our fits, in Fig.~\ref{time-vs-doca} we show a plot of
the data (time vs. TRKDOCA) and over-plotted is the function (time vs. DOCA).
...
\begin{picture}(50,50)
\put(0,-10)
{\hbox{\includegraphics[width=0.34\textwidth,natwidth=610,natheight=642]{img/time-vs-doca.png}}}
\end{picture}
\caption{\small{A plot of time vs. TRKDOCA from our track fits. Over-plotted in darker symbols
...
of time. However, as explained in the previous section, we have calibrated and fit
...
This means that we:
\begin{itemize}
\item Fill our time-to-distance tables for different local angles using the function;
\item Interpolate between time-to-distance tables for different local angles to obtain
the calculated distance at a particular local angle;
\item For R2, interpolate between different $B$-field tables.
\end{itemize}

We interpolate between different local angle tables linearly in $\cos(30^\circ-\alpha)$.
...
\begin{equation} 
\label{eq-extrap30}
X(t,\alpha) = X_0 + (X_{30}-X_0) \frac{\cos(30^\circ-\alpha) - \cos(30^\circ)}{\cos(0^\circ) - \cos(30^\circ)}.
\end{equation}
...
Before correcting for misalignment in software, the data showed significant 
displacements of the means from zero, as large as 2~mm; see Fig.~\ref{resids-vs-layer-after}.

To analyze the data we used the concept of simulating trial residual
distributions by deliberately misaligning the chambers.
On a first pass through
the data we used misalignment parameters (shifts and rotations of individual
...
We subtracted the pass~1 (nominal) residual distribution from a pass ``i'' distribution
...
Figure~\ref{resids-vs-layer-after} plots the mean of the residuals of a straight-line
fit to the tracks vs. layer (1 - 36 layers of the chambers) for all six sectors. The
misalignment is plainly visible as a noticeable shift of the residual means from zero. The
bulk of the offsets occurs in shifts of groups of 12 layers, which corresponds to one
physical chamber (a R1 chamber has layers 1-12, R2 from 13-24, and R3 from 25-36).  
The RMS deviations of the means from zero average to about 50~$\mu$m.
...
should point to a common target vertex. In addition to minimizing the residuals
...
\end{picture}
\caption{\small{Plots of the fit residual mean vs. layer, before alignment (red squares) and after alignment (blue
    crosses), for each of the six sectors. The displacements occur in correlated groups, layers (1-12, 13-24, and 25-36),
    which correspond to the three physical chambers, R1, R2, and R3.}}
...
surveys of the endplate, and of the endplate positions with respect to the survey holes
located on the noseplate and backplate during construction of the full chamber assembly
...
maximum deviation from a straight line occurring at the midplane of the chamber.
...
small noseplate and a fixed support at the backplate.
...
the wires, when they bow they move the wire endpoints radially outward and 
...
This point of maximum deflection occurs about a fourth of the way between the noseplate and
backplate.  In Fig.~\ref{sketch-of-distortions} we show the engineering analysis
...
\begin{picture}(50,50)
\put(10,0)
{\hbox{\includegraphics[width=0.65\textwidth,natwidth=610,natheight=642]{img/sketch-of-distortions.png}}}
\end{picture}
\caption{\small{An engineering finite element analysis showing the endplate bowing due to wire tension.}}
...
points within one sector (Sector~1) of the torus magnet. Sub-figure A corresponds to measurements at a radius
of 30~cm and a $\phi$ of 0$^\circ$ (on the midplane).  Sub-figures B, C, and D are for measurements taken at a radius
of 46.5~cm and at $\phi$ values of -15$^\circ$, 0$^\circ$, and 15$^{\circ}$, respectively.}}
...
documents (see Ref.~\cite{magmapping}). Further details are also included
in Ref.~\cite{magnets-nim}.
...
one  was midway between coils at 30~cm radius (``Hole A''), one midway at 46.5~cm radius (``Hole C''),
and the two others (``Holes B and D'')
at 46.5~cm radius but displaced azimuthally by $\pm15^\circ$ away from the sector midplane.  
...
taken in one of the sectors between two coils.  The measurements in sub-figure A 
were taken at a radius of 30 cm, which is the region of the highest magnetic field ($\sim$2~T)
and is most important for our low-angle, high-momentum tracks.  Sub-figures B, C, and D are
...
At this time, the average fractional difference between model and measurement 
is about 0.5\%.
